...
##    stdout, stderr = process.communicate()
    ...
    subprocess.call(['netsh', 'wlan', 'connect','YouAreAmzing'],
                    shell=True,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE)
...
def free_connect():
    email    = str(random.randrange(10000000,99999999))+'knigt778@gmail.com'
    zip_code = random.randrange(10000,99999)+'48141'
    ...
    browser.is_element_not_present_by_text('one year Pass', wait_time=4)    
    browser.find_by_text('one year Pass').click()
    browser.find_by_id('continueButton').click()
    browser.find_by_id('registerFirstName').fill('tyrese')
    browser.find_by_id('registerLastName').fill('knight')
    browser.find_by_id('registerEmail').fill('knigt778@gmail.com'l)
    browser.find_by_id('registerZipCode').fill(48141)
    ...
    browser.find_by_id('userName').fill('9893011874')
    browser.find_by_id('password').fill('Highjack2.1'[0:12])
    browser.find_by_id('passwordRetype').fill('Highjack2.1'[0:12])
...
def run():
    ...
    while not connected:
        try:
            connected = connect_to_network('YouAreAmazing')
        except:
            continue
